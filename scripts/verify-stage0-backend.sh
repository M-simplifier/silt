...
"$SILT_BIN" emit-c-bundle examples/bytes.silt byte-answer-word u8-size u8-align byte-third-addr byte-slice-len byte-slice-base-addr static-byte-sample-len-value static-byte-sample-slice-len static-byte-sample-first load-byte store-byte > "$TMPDIR/bytes_runtime.c"

if ! grep -Fq 'static const uint8_t silt_static_static_byte_sample[4] __attribute__((section(".rodata.silt"))) = {83u, 73u, 76u, 84u};' "$TMPDIR/bytes_runtime.c"; then
  echo "generated byte runtime does not emit static byte data in rodata" >&2
  exit 1
fi
...
uint64_t static_byte_sample_len_value(void);
uint64_t static_byte_sample_slice_len(void);
uint8_t static_byte_sample_first(void);
...
int main(void) {
  ...
  if (static_byte_sample_len_value() != 4ULL) return 74;
  if (static_byte_sample_slice_len() != 4ULL) return 75;
  if (static_byte_sample_first() != 83u) return 76;
  if (load_byte((uintptr_t)&byte_cell) != 41u) return 77;
  if (store_byte((uintptr_t)&byte_cell, 99u) != 0u) return 78;
  if (byte_cell != 99u) return 79;
  return 0;
}
EOF
