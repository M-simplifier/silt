...
cabal run silt -- build hosted-copy
cabal run silt -- run hosted-copy -- /tmp/silt-hosted-file.txt /tmp/silt-hosted-copy.txt
cabal run silt -- build hosted-ascii-trim
cabal run silt -- run hosted-ascii-trim -- /tmp/silt-hosted-file.txt /tmp/silt-hosted-trim.txt
...
cabal run silt -- build hosted-stdin-lf-count
printf 'alpha\nbeta\n' | cabal run silt -- run hosted-stdin-lf-count
cabal run silt -- build hosted-byte-drop
printf 'axbxxc\n' | cabal run silt -- run hosted-byte-drop -- x
cabal run silt -- build hosted-source-hygiene
cabal run silt -- run hosted-source-hygiene -- examples/hosted-byte-drop.silt
cabal run silt -- test
...
cabal run silt -- test list-recursion-test
```

`silt new NAME` は現在のdirectoryに `Silt.pkg` がなくても新しいpackage directoryを作ります。それ以外のpackage commandはカレントディレクトリの `Silt.pkg` を読みます。現在のpublic claimは、単一local package、明示的な `bin` / `test` target、no-argument hosted entry function、`silt new NAME` による単純なscaffold生成、`silt doc` によるmanifest由来HTML生成、`silt run [TARGET] -- ARG...` によるhosted process argumentの受け渡し、名前を指定した単一のenvironment lookup、bin entryの結果をprocess statusとして伝播すること、明示的な `TextView` path/bodyによるfirst-orderなhosted file write、明示的な `TextView` pathからhost-ownedな `TextView` へ読むfirst-orderなhosted file readとread-status観測、host-ownedな `TextView` へstdin全体を読む狭いhosted stdin read、`static-bytes` 用の狭いbyte string literal、`ByteSlice` / `TextView` 上のbyte-wise equality、prefix check、suffix check、狭いbyte/text substring find/contains、`U8` 上のASCII byte predicate、明示的な `ByteSlice` / `TextView` 上の狭いall-ASCII class check、空入力、非数字、overflowを拒否する狭いASCII decimal `U64` parser、caller-providedな `AsciiDecimalU64Buffer` にASCII decimal `U64` を書いて明示的な `TextView` として返すformatter、length/map/filter/any/all/find/count/append/reverse/fold helperを含むclosed `List` structural recursionに限られます。root packageには、stdlib seedの `TextView` と `host-write-text` で `SILT` を出力する `hosted-hello`、`argv[1]` を出力する `hosted-echo`、`SILT_HOSTED_ENV` を出力する `hosted-env`、`argv[1]` の有無をprocess statusで返す `hosted-exit`、`argv[1]` のpathへ固定bodyを書く `hosted-write-file`、`argv[1]` のpathから読んだ内容をstdoutへ書く `hosted-cat`、`argv[1]` のpathから読んだ内容を `argv[2]` のpathへ書く `hosted-copy`、fileの両端ASCII whitespaceをtrimしてfile/stdoutへ出す `hosted-ascii-trim`、process args、file readとread-status観測、ASCII decimal parse/format、file write、stdout、process statusを一つの経路で組み合わせる `hosted-size-report`、複数file read、first-byte split、ASCII whitespace trim、static key compare、ASCII decimal parse/format、file write、stdout、stderr diagnostics、process statusを一つの経路で組み合わせる `hosted-config-report`、fileまたはstdinのliteral LF byte countを出力する `hosted-lf-count` / `hosted-stdin-lf-count`、stdinからone-byte argumentと一致するbyteを落としてstdoutへ出す `hosted-byte-drop`、file内のNUL/CR byte hygieneを報告する `hosted-source-hygiene`、one-byte argumentを検証してfile内のbyte containmentを報告する `hosted-byte-search`、non-empty needleを検証してfile内のbyte/text substring containmentを報告する `hosted-text-search`、stdlibの正規化例を確認する `stdlib-test`、List構造再帰を確認する `list-recursion-test`、静的byte-backed textの等価比較を確認する `text-eq-test`、prefixを確認する `text-prefix-test`、suffixを確認する `text-suffix-test`、substring searchを確認する `text-search-test`、ASCII byte predicateを確認する `ascii-test`、ASCII slice/text predicateを確認する `ascii-slice-test`、ASCII decimal parseを確認する `ascii-decimal-u64-test`、ASCII decimal formatを確認する `ascii-decimal-u64-format-test` が含まれます。dependency、workspace、lockfile、package publishing、package resolution、selectable package template、source comment extraction、type-signature docs、cross-package docs、environment enumeration/mutation、現在のsuccess/failure statusを超える詳細なfile-read error分類、streaming stdin、append mode、directory operation、path library、in-place file editing、process spawning、signals、stdout/stderr abstraction、general hosted IO、multi-entry config format、generic CLI/config parser、multi-file source traversal、CRLF normalization、Unicode category、locale-sensitive behavior、case conversion、UTF-8 validation、`static-bytes` のbyte literal convenienceを超えるgeneral string literal/string type、動的文字列、regexやUnicode-aware search tool、現在の狭いall-ASCII class check、狭いbyte/text substring find/contains、ASCII decimal `U64` parser、ASCII decimal `U64` formatterを超えるsubstring/search/general scanning API、任意のuser-defined dataに対するstructural recursion、dependent list induction、generic ADT runtime representation、open-term List law proof、broad sequence library、符号、基数prefix、separator、whitespace trimming、詳細なparse error分類、符号付きformat、radix format、padding、alignment、locale formatting、parser/formatter combinator libraryはまだclaimしません。

`hosted-byte-drop` は一致byteのdropだけを扱い、`tr`-style byte translationやstreaming filterはまだclaimしません。
...
cabal run silt -- build hosted-source-hygiene
cabal run silt -- run hosted-source-hygiene -- examples/hosted-byte-drop.silt
cabal run silt -- build hosted-byte-search
cabal run silt -- run hosted-byte-search -- /tmp/silt-hosted-file.txt A
cabal run silt -- build hosted-text-search
cabal run silt -- run hosted-text-search -- /tmp/silt-hosted-file.txt alpha
cabal run silt -- build hosted-copy
cabal run silt -- run hosted-copy -- /tmp/silt-hosted-file.txt /tmp/silt-hosted-copy.txt
cabal run silt -- build hosted-ascii-trim
cabal run silt -- run hosted-ascii-trim -- /tmp/silt-hosted-file.txt /tmp/silt-hosted-trim.txt
...
cabal run silt -- test text-search-test
