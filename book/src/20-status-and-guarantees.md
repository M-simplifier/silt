...
- UTF-8 validation、CRLF normalization、split-all line API、汎用文字列、配列、dynamic slice、allocator-backed byte/text buffer、現在のfirst-byte find/contains/split helper、LF-oriented first-line split helper、狭いall-ASCII class check、狭いASCII whitespace trim、ASCII decimal `U64` parser、ASCII decimal `U64` formatterを超えるsubstring/search/general scanning API、collation、text-normalization API
